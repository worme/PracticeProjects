class Pokemon:
    ...
    def __init__(self, name, level, element_type, max_health, current_health, is_knocked_out):
        self.name = name
        self.level = level
    ...
    def knock_out(self):
        """Determines if a pokemon has been knocked out by assessing if the pokemon's current health has fallen to 0 or
        below."""
    ...
    def attack(self, other_poke):
        """Takes a pokemon as an argument and determines the damage caused from one pokemon to the one passed. This is
...
class Trainer:
    ...
    def attack_trainer(self, other_trainer):
        """Takes a trainer object as an argument and uses Pokemon class with attack method to attack the other trainer's
         active pokemon."""
        print(f"{self.name} is attacking {other_trainer.name}")
